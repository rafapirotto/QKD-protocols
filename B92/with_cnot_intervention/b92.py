...
def b92(alice_bits=None, bob_bits=None, eve_bits=None, vector=None):
  ...
  size = len(alice_bits)
  # we double the length because Eve needs an extra qubit due to the nature of the CNOT attack
  number_of_qubits = size * 2
  ...
  cnot_attack(circuit)
  insert_measurements_according_to_base_eve(eve_bases, circuit)
  circuit.barrier()
  eve_measurements = get_measurements_result(backend, circuit, shots, accuracy, size)

  insert_measurements_according_to_base_bob(bob_bases, circuit)

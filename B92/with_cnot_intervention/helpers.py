...
def insert_measurements_according_to_base_eve(eve_bases, circuit):
    for i in range(len(eve_bases)):
        base = eve_bases[i]

        if base == Z_BASE:
            measure_in_z(circuit, 2*i + 1)
        elif base == X_BASE:
            measure_in_x(circuit, 2*i + 1)


def insert_measurements_according_to_base_bob(bob_bases, circuit):
    for i in range(len(bob_bases)):
        base = bob_bases[i]

        if base == Z_BASE:
            measure_in_z(circuit, 2*i)
        elif base == X_BASE:
            measure_in_x(circuit, 2*i)
